...
function savePopupStateExt() {
  ...
  popupExt.style.maxHeight = '600px';
        ...
        // SVG icon for dictionary button (can be replaced with any icon you prefer)
   ...
   xmlns:svg="http://www.w3.org/2000/svg"><sodipodi:namedview
     ...
     id="layer1"><path
       ...
       transform="matrix(0.89266763,0,0,0.89266763,5.5336428,8.0597213)"><path
        ...
        // iframeExt.sandbox = 'allow-scripts allow-same-origin allow-forms'; // Restrict iframeExt capabilities for security
    ...
    function startDragExt(e) {
        ...
        if (isFirstDrag) {
            ...
            popupExt.style.transform = 'none';  // убираем transform, который центрирует окно
    ...
    document.addEventListener('touchend', stopDragExt);
        ...
        function closePopupExt(event) {
            ...
            popupExt.style.display = 'none';
            overlayExt.style.display = 'none';
            iframeExt.src = ''; // Clear iframeExt content
        }
        ...
        // Reset popupExt size to default
        function resetSizeExt() {
            popupExt.style.width = '80%';
            popupExt.style.height = '80%';
            popupExt.style.top = '50%';
            popupExt.style.left = '50%';
    ...
    function handleClickExt(event) {
        ...
        if (clickedWord) {
            const processedWord = processWordExt(clickedWord);
            const url = `${customDictUrl}${encodeURIComponent(processedWord)}`; // Use custom URL
            iframeExt.src = url; // Load URL in iframeExt
            popupExt.style.display = 'block';
            overlayExt.style.display = 'block';
            openBtnExt.href = `${dhammaGiftURL}${encodeURIComponent(processedWord)}${dgParams}`; // Set open button URL
